...
def train(train_loader, val_loader, distance_fn, model, criterion, optimizer, lr_scheduler, epochs, device, writer, metrics,
          train_stats, val_stats, log_dir, new_model_attention=False, model_devise=False, kda=False, cjme=False, args=None):
    best_loss = None
    best_score = None

    for epoch in range(epochs):
        train_loss = train_step(train_loader, model, criterion, optimizer, epoch, epochs, writer, device, metrics,
                                train_stats, new_model_attention, model_devise, kda, cjme, args)
        val_loss, val_hm = val_step(val_loader, model, criterion, epoch, epochs, writer, device, metrics, val_stats,
                                     new_model_attention, model_devise,kda,cjme, args)
...
def train_step(data_loader, model, criterion, optimizer, epoch, epochs, writer, device, metrics, stats,
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        if new_model_attention==False and model_devise==False and kda==False:
            ...
            embeddings, mapping_dict = data_loader.dataset.zsl_dataset.map_embeddings_target
            act_ids = torch.tensor(list(data_loader.dataset.targets_set)).cuda()
            for i in range(inputs[2].shape[0]):
                inputs[2][i] = mapping_dict[(inputs[2][[i]]).item()]
            # input_features = torch.cat((inputs[1], inputs[0]), 1)
            # output_final, pre_attri, attention, pre_class, attribute = model(inputs[1], inputs[0], embeddings)
            # loss, loss_details=criterion(model, output_final, pre_attri, pre_class, inputs[3] , inputs[2])
            output_final, pre_attri, text_embeds, text_embeds_all = model(inputs[1], inputs[0], x_p_num, act_ids, is_train=True)
            loss, loss_details= criterion(model, output_final, pre_attri, text_embeds_all, inputs[2])
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        print(str(batch_idx)+'/'+str(batch_lenth)+'   loss '+str(loss.item()), end='', flush=True)
        time.sleep(0.1)  # 添加适当的延迟，以便观察到输出的变化
        print('\r', end='')
...
def val_step(data_loader, model, criterion, epoch, epochs, writer, device, metrics, stats,
             new_model_attention,model_devise,kda,cjme, args=None):
    ...
    with torch.no_grad():
        ...
        hm_score = 0
        for batch_idx, (data, target) in enumerate(data_loader):
            print(str(batch_idx)+'/'+str(batch_lenth), end='', flush=True)
            time.sleep(0.1)  # 添加适当的延迟，以便观察到输出的变化
            print('\r', end='')
            ...
            if new_model_attention==False and model_devise==False and kda==False:
                ...
                # output_final, pre_attri, attention, pre_class, attribute = model(inputs[1], inputs[0], embeddings)
                # loss, loss_details = criterion(model, output_final, pre_attri, pre_class, inputs[3], inputs[2])
                output_final, pre_attri, text_embeds, text_embeds_all = model(inputs[1], inputs[0], x_p_num, act_ids, is_train=False)
                loss, loss_details = criterion(model, output_final, pre_attri, text_embeds_all, inputs[2])
        ...
        logger.info(
            ...
            f"Iteration: {iteration}\t"
            f"Loss: {batch_loss:.4f}\t"
            f"Seen: {100*seen_score:.4f}\t"
            f"Unseen: {100*unseen_score:.4f}\t"
            f"ZSL score: {100*zsl_score:.4f}\t"
            f"HM: {100*hm_score:.4f}"
        )
