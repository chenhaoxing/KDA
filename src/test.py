...
def test(eval_name, val_dataset, test_dataset, model_A, model_B, device, distance_fn,
          args=None, new_model_attention=False, devise_model=False, kda=False, save_performances=False):
    ...
    test_evaluation = _get_test_performance(val_dataset=val_dataset, test_dataset=test_dataset, model_A=model_A,
                                            ...
                                            kda=kda, save_performances=save_performances)
...
def _get_test_performance(val_dataset, test_dataset, model_A, model_B, device, distance_fn, args, new_model_attention, devise_model, kda, save_performances=False):
    logger = logging.getLogger()
    # if  new_model_attention or devise_model or kda:
    ...
    if new_model_attention or devise_model or kda:
        test_evaluation = evaluate_dataset_baseline(test_dataset, model_B, device, distance_fn,
                                                    args=args,
                                                    new_model_attention=new_model_attention,
                                                    model_devise=devise_model,
                                                    kda=kda, save_performances=save_performances)
    else:
        test_evaluation = evaluate_dataset(test_dataset, model_B, device, distance_fn,args=args)
