...
class Distribution_loss(nn.Module):
    ...
    def wasserstein_distance(self, feature1, feature2):
        mean1 = torch.mean(feature1)
        mean2 = torch.mean(feature2)
        centered_matrix1 = feature1 - mean1
        centered_matrix2 = feature2 - mean2
        
        # 计算特征向量的协方差矩阵
        cov1 = torch.matmul(centered_matrix1.t(), centered_matrix1) / feature1.size(0)
        cov2 = torch.matmul(centered_matrix2.t(), centered_matrix2) / feature2.size(0)
        
        # Use 'torch.sqrtm' function to compute square roots of matrices if available
        cov1_sqrt = torch.sqrtm(cov1) if hasattr(torch, 'sqrtm') else torch.sqrt(cov1)
        cov2_sqrt = torch.sqrtm(cov2) if hasattr(torch, 'sqrtm') else torch.sqrt(cov2)
        
        mean_distance = torch.pow(mean1 - mean2, 2)
        cov_distance = torch.pow(torch.norm(cov1_sqrt - cov2_sqrt, p='fro'), 2)
        
        # Square the result for the final Wasserstein distance (as per instruction)
        wasserstein_dist = torch.pow(mean_distance + cov_distance, 2)
    ...
    def forward(self, x1, x2):
        ...
        for i in range(b):
            total_dis += self.wasserstein_distance(x1[i], x2[i])
        
        # Average out the total Wasserstein distance
        return total_dis / b

class KDA_Loss(nn.Module):
    def __init__(self, lamda=0):
        super().__init__()
        self.criterion = nn.CrossEntropyLoss()
        self.criterion_regre = nn.MSELoss()
        self.lamda = lamda
    def forward(self, model, output, pre_attri, label_a,
                label_v):     
        # label_a(text_embedding)  256, 300 label_v 视频类别id 256 
        # loss_xe =self.criterion(output, label_v)
        # loss = loss_xe   
        loss_align = self.criterion_regre(label_a, pre_attri['final'])
        loss = self.lamda * loss_align
